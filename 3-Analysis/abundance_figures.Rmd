...
```{r}
# for subject-level, we want to know how many unique hits they have within the entire coassembly (do not double-count contigs that show up in different samples)
subject_unique_subunit_RPKM <- samples_unique_gene_casette %>%
  filter(single_closest_ref != "Moorella_thermoacetica_Y72_copy_2__MooTher5") %>%
  group_by(subject, contig) %>%
  slice_max(RPKM, n = 1, with_ties = FALSE) %>%
  group_by(subject, sample_type) %>%
  summarize(number_hits=n()) %>%
  group_by(sample_type) %>%
  summarize(median_hits=median(number_hits), q_25=quantile(number_hits, probs=0.25), q_75=quantile(number_hits, probs=0.75))
```
...
ggsave("dsrAB_RPKM_combine_species_wCollectionTimes.png",
       ...
       dpi = 600)
```
## sampling variability for subject 9 
```{r}
samples_unique_subunit_RPKM %>%
  #remove moorella because its distance from the queries is 7.5 (all others are less than 2.5)
  filter(single_closest_ref != "Moorella_thermoacetica_Y72_copy_2__MooTher5") %>%
