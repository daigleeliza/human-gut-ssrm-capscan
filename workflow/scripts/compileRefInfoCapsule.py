...
## For capsule/stool samples, hit_id includes the coassembly and query_id (contig + gene location)
## biosample_id is the coassembly (e.g., Capsule_1 or Stool_9)
## removed lines 50-62 from compileRefInfo.py

INPUT:
    queryDistanceInfoCSV :
        CSV file with hit sequence info for unique hit sequences only (no duplicates)
  rm ### biosampleJSON :
  rm ###     key = sample_ID, value "query_IDs" = WGS accession number
    scoreThresholdCSV :
        CSV file with hit sequences for all hits that pass the score threshold

OUTPUT:
    closestRefInfo_allScoreThresholdHits_capsule.csv
"""
...
# iterate through scoreThreshold CSV and add reference info for each row
...
with open(scoreThresholdCSV, 'r') as f:
    ...
    with open(outputCSV, 'w') as f:
        ...
        for row in scoreThreshold_reader:
            ...
            biosample_id = hit_id.split(".")[0]  # biosample_id is the coassembly ID
            ...
            with open(queryDistanceCSV, 'r') as f:
                ...
                writer_row = [hit_id, query_id, biosample_id, closest_ref, distance, SRM_present]
