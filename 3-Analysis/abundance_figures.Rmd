...
median_RPKM_by_subject <- samples_unique_subunit_RPKM %>%
  ...
  group_by(subject_type, single_closest_ref) %>%
  summarize(median_RPKM=round(median(RPKM),2))%>%
  pivot_wider(
    names_from = single_closest_ref,
...
# normalize ??
subject_RPKM_sums <- rowSums(median_RPKM_by_subject)
matrix_norm <- (median_RPKM_by_subject/subject_RPKM_sums)*100 

# transform  
matrix_norm_log <- (log10(matrix_norm+1)) %>%
  as.matrix()
matrix_norm_log[is.infinite(matrix_norm_log)] <- NA
```
...
```
```{r}
# Extract Type from rownames (everything after the underscore)
row_types <- sub(".*_", "", rownames(matrix_norm_log))

# Create color function with gray for NA
col_fun <- colorRamp2(
  seq(min(matrix_norm_log, na.rm = TRUE), 
      max(matrix_norm_log, na.rm = TRUE), 
      length.out = 50),
  viridis(50, begin = 0.1, end = 0.95)
)

# Create heatmap with row splitting by Type

hm <- Heatmap(
  matrix_norm_log,
  name = "Log10(Norm %)",
  col = col_fun,
  na_col = "gray80",  # Color for NA values
  
  # Split rows into 4 panels based on Type
  row_split = row_types,
  row_title_rot = 0,
  row_gap = unit(2, "mm"),
  ...
  heatmap_legend_param = list(
    ...
    at = seq(min(matrix_norm_log, na.rm = TRUE), 
             max(matrix_norm_log, na.rm = TRUE), 
             length.out = 5)
  )
)
