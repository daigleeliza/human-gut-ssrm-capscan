...
rule removeBootstrapValues:
	input:
		original=join(config["raxmlOutputDir"], f"RAxML_originalLabelledTree.{config['treeFileExtension_frag_capsule']}")
		labelled=join(config["raxmlOutputDir"], f"RAxML_labelledTree.{config['treeFileExtension_frag_capsule']}")
	output:
		original=join(config["raxmlOutputDir"],"RAxML_originalLabelledTree_noBootstrap_capsule.newick"),
		labelled=join(config["raxmlOutputDir"],"RAxML_labelledTree_noBootstrap_capsule.newick")
	resources:
		time=10,
		mem_mb=500
	shell:
