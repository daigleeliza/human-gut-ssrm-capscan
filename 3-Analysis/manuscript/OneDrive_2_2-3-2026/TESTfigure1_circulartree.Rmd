...
```{r}
#root tree
ref_tree = read.tree(paste0(dataDir, "/RAxML_originalLabelledTree_noBootstrap_wKarthikSeqs_rooted.newick"))
...
```{r}
taxa_table <- read_csv(paste0(dataDir,"/taxa_table.csv"))
```
...
```{r}
closestRefInfo_allScoreThresholdHits_singleRefOnly <- read_csv(paste0(dataDir, "closestRefInfo_allScoreThresholdHits_wSingleClosestRef.csv"))

hit_otus <- unique(subset(closestRefInfo_allScoreThresholdHits_singleRefOnly$single_closest_ref, closestRefInfo_allScoreThresholdHits_singleRefOnly$distance < 2.5 & !is.na(closestRefInfo_allScoreThresholdHits_singleRefOnly$single_closest_ref)))
...
tree + 
  ...
  geom_cladelabel(node = MRCA(pruned_tree, nitrospirae_node), label = "Nitrospirae supercluster", align = TRUE, barsize = 1, fontsize = 3.5, fill = "white") +
...
# desulfovibrionaceae_node <- intersect(as.vector(subset(taxa_table$OTU, taxa_table$level3_taxon == "Desulfovibrionaceae")), pruned_tree$tip.label)
# 
desulfovibrionaceae_tax <- subset_taxa(taxa_table, level3_taxon == "Desulfovibrionaceae")
# 
