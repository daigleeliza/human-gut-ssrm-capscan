...
# if you wanted to see what the Ley sequences are for 2018 seq you need to read in the fragment insertion with the 2018 seqs
...
ley3_2015_identity <- closestRefInfo_allScoreThresholdHits_capsule_2015 %>% filter(query_id %in% ley3$query_id)
```
...
# how do counts of dsrAB compare to counts of asrABC
```{r}
...
## mixed-effects model for predicting sample_type based on ratio not quite right because the residuals are not normal
```{r}
library(lmerTest)
model <- lmerTest::lmer(log(ratio_dsr_asr) ~ sample_type + (1|subject), data = plot_ratios)
summary(model)
...
# for seeing which samples definitely have hits so that we can figure out which are assemblies!
samples_of_interest_all_species <- samples_w_subj %>%
  filter(merged_subject_id %in% subject_OTU_matrix$subject_id)
...
# comparing bilophila abundance in SI to stool
```{r}
