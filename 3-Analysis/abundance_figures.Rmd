...
# plot RPKG for unique genes (each hit would be one subunit) in each sample to compare between location; facet by species and subject
...
ggplot(plot_capsule_differences, 
       aes(x = Type, y = log10(sum_RPKG_per_1000), group = subject)) +
...
matrix_sample_type_log <- round(log10(RPKG_by_sample_type+1),4) %>%
  as.matrix()
```
...
```{r}
png(paste0(figureDir, "heatmap_split.png"), width = 8, height = 6, units = "in", res = 600)
