...
```{r}
t_test_results <- all_genes %>%
  mutate(log_sum = log10(sum)) %>%
  group_by(sample_type, subject) %>%
  summarise(
    t_test = list(
      tryCatch(
        t.test(log_sum ~ gene_cluster),
  ...
  mutate(
    statistic    = sapply(t_test, function(x) if (!is.null(x)) x$statistic  else NA),
    p_value      = sapply(t_test, function(x) if (!is.null(x)) x$p.value    else NA),
    df           = sapply(t_test, function(x) if (!is.null(x)) x$parameter  else NA),
    mean_asr     = sapply(t_test, function(x) if (!is.null(x)) x$estimate[1] else NA),
    mean_dsr     = sapply(t_test, function(x) if (!is.null(x)) x$estimate[2] else NA)
  ) %>%
  select(-t_test) %>%
...
# Create annotation dataframe from t-test results
signif_labels <- t_test_results %>%
...
# Get y position for brackets (just above the max value per group)
y_positions <- all_genes %>%
  mutate(log_sum = log10(sum)) %>%
  group_by(sample_type, subject) %>%
  summarise(y_pos = max(log_sum, na.rm = TRUE) + 0.2, .groups = "drop")

signif_labels <- signif_labels %>%
  left_join(y_positions, by = c("sample_type", "subject"))
```
...
all_genes %>%
  ...
  geom_signif(
    data = signif_labels,
    aes(
      xmin = sample_type,     # same x since comparing within sample_type
      xmax = sample_type,
      annotations = label,
      y_position = y_pos
    ),
