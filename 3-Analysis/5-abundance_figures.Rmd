...
model_df <- plot_capsule_differences %>% 
  mutate(log_abundance = log10(median_RPKG_per_1000))

model <- lmerTest::lmer(log_abundance ~ Type + combine_species + (1|subject), 
              data = model_df)
...
# testing reg anova
summary(aov(log_abundance ~ Type,
  data = model_df
))
```
...
# in one plot
```{r}
#extract species, subject, RPKG columns to make a matrix that has subjects as row names and species as column names
RPKG_by_sample_type <- samples_unique_subunit_RPKG %>%
...
# just change the df to RPKG_by_sample_type to get original plot
...
(RPKG_by_sample_type_MEDIAN %>%
  filter(grepl("Stool", rownames(.))) %>%
  select(Bilophila_species) %>%
  sum())
(RPKG_by_sample_type_MEDIAN %>%
  filter(grepl("Capsule", rownames(.))) %>%
  select(Bilophila_species) %>%
  sum())
...
# get number for results
(RPKG_by_sample_type_MEDIAN %>%
  filter(grepl("Stool", rownames(.))) %>%
  select(Desulfovibrio_piger) %>%
  sum())/(RPKG_by_sample_type_MEDIAN %>%
  filter(grepl("Stool", rownames(.))) %>%
  select(Bilophila_species) %>%
  sum())
```
...
```{r}
#old version that was biased by number of capsule samples
# bilophila_abun <- samples_unique_subunit_RPKG %>%
#   # combine d. piger and bilophila species
#   mutate(combine_species = case_when(
#     single_closest_ref %in% c("Desulfovibrio_piger__DslPige3", "Desulfovibrio_piger__DslPige4") ~ "Desulfovibrio_piger",
#     single_closest_ref %in% c("Bilophila_wadsworthia__BilWads6", "Bilophila_sp._4_1_30__BilSpec3") ~ "Bilophila_species",
#     TRUE ~ single_closest_ref)) %>%
#   group_by(subject, sample_type, combine_species) %>%
#   summarize(sum_RPKG_per_1000=sum(RPKG)*1000) %>%
#   select(subject, sample_type, combine_species, sum_RPKG_per_1000) %>%
#   filter(combine_species == "Bilophila_species") %>%
#   pivot_wider(names_from = sample_type, values_from = sum_RPKG_per_1000) %>%
#   group_by(subject) %>%
#   summarize(ratio=Capsule/Stool)
...
```{r}
## this was originally heatmap_split
