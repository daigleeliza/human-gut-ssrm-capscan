...
inset_firmicutes <- firmicutes_subset_tree_plot %<+%  
  ...
  geom_tiplab(aes(subset = label %in% subjects_per_species$single_closest_ref,
                    label = species_labels[label]), size = 3, align = F, hjust = -.55,
              angle = 0) +
...
inset_desulfo <- desulfo_subset_tree_plot %<+%
  ...
  geom_tiplab(aes(subset = label %in% subjects_per_species$single_closest_ref,
                    label = species_labels[label]), size = 3, align = F, hjust = -0.15,
              angle = 0) +
...
```{r}
# to make this plot, change hjust to -0.15 for firmicutes and both label sizes to 2
